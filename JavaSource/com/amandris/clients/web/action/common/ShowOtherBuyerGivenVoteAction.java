...
public class ShowOtherBuyerGivenVoteAction extends AbstractAction {
	...
	public ActionForward execute( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception 
	{
		...
		otherUserGivenVoteDataView = voteFacade.getOtherBuyerGivenVoteDataByBuyerId( DBUtils.parseInteger( request.getParameter( "id"), 0), "date", false, Boolean.TRUE, 0, Integer.MAX_VALUE, locale);
		
		if( otherUserGivenVoteDataView == null)
			return error( "error.couldNotShowBuyerVote", request, mapping);
		
		request.setAttribute( Constant.OTHER_BUYER_GIVEN_VOTE_DATA_OBJECT_NAME, otherUserGivenVoteDataView);
