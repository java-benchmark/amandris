...
public class SetSellerEditCalendarAction extends AbstractAction {
	...
	public ActionForward execute( ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception 
	{
		...
		selectedDate 	= DBUtils.parseDate( request.getParameter( "date"));
		startDate 		= request.getParameter( "startDate");
		
		if( selectedDate == null)
			return error( "error.couldNotEditCalendar", request, mapping);
		
		hour = DBUtils.parseInteger( request.getParameter( "hour"), -1).intValue();
		
		if( hour < 0 || hour > 23)
			return error( "error.couldNotEditCalendar", request, mapping);
				
		status = DBUtils.parseInteger( request.getParameter( "status"), -1).intValue();
				
		calendarFacade.setCalendarHourStatus( sellerSession.getId(), selectedDate, hour, status, locale);
		...
		session.setAttribute( Constant.EDIT_CALENDAR_OBJECT_NAME, editCalendarView);
