...
public class SetSellerPictureAction extends AbstractAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception 
	{
		...
		if( ! pictureForm.getPictureFile().getFileName().trim().equals( "")) {
			try {
				resampleImage( pictureForm.getPictureFile().getFileData(), "/pictures/smaller/seller_" + sellerSession.getId() + "_" + pictureForm.getIndex() + "_" + randomNumber + ".jpg", 55, 67, true);
				resampleImage( pictureForm.getPictureFile().getFileData(), "/pictures/original/seller_" + sellerSession.getId() + "_" + pictureForm.getIndex() + "_" + randomNumber + ".jpg", 640, 800, false);
			} catch ( Exception e) {
				return error( "error.couldNotSetPicture", request, mapping);
			}
			
			if( sellerFacade.setSellerPicture( pictureForm, sellerSession, randomNumber) == false)
				return error( "error.couldNotSetPicture", request, mapping);
